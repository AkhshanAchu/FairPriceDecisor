...
quality = int(st.selectbox(
    'Select Quality',
    ('1', '2', '3', '4', '5')))
input = st.number_input('Required Amount (Kgs) : ')
demand = int(st.number_input('Demand in area(1-10)'))

if st.button('Submit'):
    class Buyer:
        def __init__(self, season, crop, quality, input, demand):
            self.season = season
            self.crop = crop
            self.mean = 0
        ...
        def extract(self):
            ...
            if self.season == "Pre-Monsoon":
                self.data = pd.read_csv(".\pages\dataset\pre.csv")
                self.mean = self.data[self.data["commodity_name"]
                                      == self.crop]["Predicted_price"].mean()

            if self.season == "Post-Monsoon":
                self.data = pd.read_csv(".\pages\dataset\post.csv")
                self.mean = self.data[self.data["commodity_name"]
                                      == self.crop]["Predicted_price"].mean()

            if self.season == "Monsoon":
                self.data = pd.read_csv(".\pages\dataset\mon.csv")
                self.mean = self.data[self.data["commodity_name"]
                                      == self.crop]["Predicted_price"].mean()

        def requirement(self):
            self.quality = self.quality / 5
            mini = self.data[self.data["commodity_name"] == self.crop]["min_price"].mean()
            self.price = self.mean - ((self.mean - mini)*(1-self.quality))

        def decider(self):

            price_market = (self.price * self.input)
            trans = self.input
            x = np.array([price_market,trans])
            y = np.array([0.9, 1000])

           
            final_output = round(x@y.T)
            st.subheader("You can Buy with Rs. " +
                         str(final_output) + " for your Requirement!")
    ...
    G = Buyer(season, crop,quality, input, demand)
    G.all()
